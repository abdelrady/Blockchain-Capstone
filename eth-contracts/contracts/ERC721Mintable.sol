...
contract Ownable {
    ...
    modifier onlyOwner(){
        require(_owner == msg.sender, "Only owner can access this feature");
        _;
    }

    modifier validAddress(address account)
    {
        require(isValidAddress(account), "'account' must be a valid address.");
        _;
    }

    function isValidAddress(address account) internal returns(bool)
    {
...
contract Pausable is Ownable {
    ...
    event Paused(address initiator);
    event UnPaused(address initiator);
    ...
    modifier whenNotPaused(){
        require(_paused == false, "Contract must not be paused to do this action");
        _;
    }

    modifier paused(){
        require(_paused, "Contract must be paused to do this action");
        _;
    }

    function updateStatus(bool pause) public onlyOwner {
        _paused = pause;
...
//    @dev Approves another address to transfer the given token ID
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(isValidAddress(to), "Receiver must be a valid address.");
